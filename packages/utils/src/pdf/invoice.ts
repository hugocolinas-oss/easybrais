...
function drawHeader(
  ...
  fontRegular: PDFFont,
  fontBold: PDFFont,
  logo: Awaited<ReturnType<typeof embedBrandLogo>>,
  bookingCode: string,
  title: string,
  subtitle: string,
) {
  ...
  const textX = MARGIN_L + (logo ? 58 : 0);
  ...
  return { width, height, gold, white, brandGreen };
}
...
export async function generateInvoicePdf(data: InvoiceData): Promise<Uint8Array> {
  ...
  page.drawText(data.bookingCode, {
    x: width - MARGIN_R - fontBold.widthOfTextAtSize(data.bookingCode, 17),
    y: height - 104,
    size: 17,
    font: fontBold,
    color: white,
