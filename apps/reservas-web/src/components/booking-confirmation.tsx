      ...
      <div className="overflow-hidden rounded-2xl border border-cream-300/80 bg-white shadow-soft">
        ...
        <div className="px-6 pt-6 sm:px-8">
          <dl className="divide-y divide-cream-200/60 text-sm">
            <DetailRow label={t("conf.client")} value={result.customerName} />
            <DetailRow label={t("customer.email")} value={result.email} />
            <DetailRow label={t("conf.firstDate")} value={fmtDateShort(result.firstServiceDate)} />
            <DetailRow label={t("summary.transports")} value={String(result.legsCount)} />
            <DetailRow
              label={t("leg.bags")}
              value={
                pricing.totalOverweightBags > 0
                  ? `${pricing.totalBags} (${pricing.totalOverweightBags} ${t("conf.withOverweight")})`
        ...
        <div className="mx-6 mt-4 overflow-hidden rounded-xl border border-cream-300/60 bg-cream-50 sm:mx-8">
          <div className="border-b border-cream-300/40 bg-cream-100/50 px-4 py-2.5">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-brand-800/40">{t("conf.priceBreakdown")}</p>
          </div>
          <div className="space-y-1.5 px-4 py-3 text-sm">
            {pricing.subtotalAmount > 0 && (
              <div className="flex justify-between text-brand-800/60">
                <span>{t("summary.subtotal")} ({pricing.totalBags} {pricing.totalBags === 1 ? t("summary.backpack") : t("summary.backpacks")})</span>
            ...
            {pricing.discountedBags > 0 && (
              <div className="flex justify-between text-sage-700">
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="rounded bg-sage-100 px-1.5 py-0.5 text-[9px] font-bold uppercase text-sage-600">{t("conf.savings")}</span>
                </span>
