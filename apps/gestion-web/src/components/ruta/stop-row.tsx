...
import type { RouteStop } from "@/lib/route-queries";
import { toggleStopCompleted, swapStopPositions } from "@/app/(dashboard)/ruta/actions";
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="hidden shrink-0 text-right sm:block">
            {stop.booking_id ? (
              <Link href={`/reservas/${stop.booking_id}`} className="font-mono text-xs font-semibold text-brand-600 hover:underline">
                {stop.booking_code}
              </Link>
            ) : (
